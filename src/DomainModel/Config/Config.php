<?php

/*
 * This file is part of git-pull-request/git-pull-request.
 *
 * (c) Julien Dufresne <https://github.com/git-pull-request/git-pull-request>
 *
 * For the full copyright and license information, please view
 * the LICENSE file that was distributed with this source code.
 */

declare(strict_types=1);

namespace GitPullRequest\DomainModel\Config;

use GitPullRequest\DomainModel\Exception\UndefinedConfigKeyException;
use GitPullRequest\Git\Exception\RuntimeException;
use GitPullRequest\Git\Git;
use Symfony\Component\Filesystem\Filesystem;
use Symfony\Component\Yaml\Exception\ParseException;
use Symfony\Component\Yaml\Yaml;

/**
 * Manage config files.
 */
final class Config
{
    /** @var string */
    private static $fileHeader = <<<'EOT'
# This file was generated by the "git pr init" command.
# For more information, please visit https://github.com/git-pull-request/git-pull-request


EOT;
    /** @var Git */
    private $git;
    /** @var array */
    private $options;

    /**
     * Constructor.
     */
    public function __construct()
    {
        $this->git = new Git();
    }

    public function debug()
    {
        dump('local', $this->getLocalOptions());
        dump('shared', $this->getSharedOptions());
        dump('global', $this->getGlobalOptions());
        dump('merged', $this->getOptions());
    }

    /**
     * @return array
     */
    public function getGlobalOptions() : array
    {
        return $this->getFileOptions('global');
    }

    /**
     * @return array
     */
    public function getSharedOptions() : array
    {
        return $this->getFileOptions('shared');
    }

    /**
     * @return array
     */
    public function getLocalOptions() : array
    {
        return $this->getFileOptions('local');
    }

    /**
     * @param array $options
     */
    public function saveGlobalOptions(array $options)
    {
        $dump = self::$fileHeader.Yaml::dump($options, 99);
        file_put_contents(getenv('HOME').'/.git-pull-request.yml', $dump);
    }

    /**
     * @param array $options
     *
     * @throws \GitPullRequest\Git\Exception\RuntimeException
     */
    public function saveSharedOptions(array $options)
    {
        $dump = self::$fileHeader.Yaml::dump($options, 99);
        file_put_contents($this->git->getProjectRootDir().'/.git-pull-request.yml', $dump);
    }

    /**
     * @param array $options
     *
     * @throws \GitPullRequest\Git\Exception\RuntimeException
     */
    public function saveLocalOptions(array $options)
    {
        $dump = self::$fileHeader.Yaml::dump($options, 99);
        file_put_contents($this->git->getProjectRootDir().'/.git/.git-pull-request.yml', $dump);
    }

    /**
     * @param $keys
     *
     * @return string|null
     */
    public function get($keys)
    {
        $result = null;
        foreach ($keys as $key) {
            $result = $this->getOne($key);
            if (null !== $result) {
                break;
            }
        }

        return $result;
    }

    /**
     * @param string|string[] $keys
     *
     * @throws \GitPullRequest\DomainModel\Exception\UndefinedConfigKeyException
     *
     * @return string
     */
    public function mustGet($keys)
    {
        if (!is_array($keys)) {
            $keys = func_get_args();
        }

        $result = $this->get($keys);
        if (null === $result) {
            if (1 < func_num_args() || 1 < count((array) $keys)) {
                $exceptionMessage = sprintf('None of the config keys "%s" are defined.', implode('", "', $keys));
            } else {
                $exceptionMessage = sprintf('The config key "%s" is not defined.', reset($keys));
            }
            throw new UndefinedConfigKeyException(
                $exceptionMessage.PHP_EOL.'You need to set up your repository by running git pr init.'
            );
        }

        return $result;
    }

    /**
     * @param string $key
     *
     * @return string|null
     */
    private function getOne(string $key)
    {
        $options  = $this->getOptions();
        $keyParts = explode('.', $key);

        do {
            $firstPart = array_shift($keyParts);

            if (!array_key_exists($firstPart, $options)) {
                break;
            }
            $options = $options[$firstPart];
        } while (0 < count($keyParts));

        return is_string($options) ? $options : null;
    }

    /**
     * @return array
     */
    private function getOptions()
    {
        if (null === $this->options) {
            $this->options = $this->computeOptions();
        }

        return $this->options;
    }

    /**
     * @return array
     */
    private function computeOptions() : array
    {
        return array_merge(
            $this->getGlobalOptions(),
            $this->getSharedOptions(),
            $this->getLocalOptions()
        );
    }

    /**
     * @param string $fileType
     *
     * @return array
     */
    private function getFileOptions(string $fileType) : array
    {
        $files = $this->getFiles();
        if (!array_key_exists($fileType, $files)) {
            return [];
        }

        return $this->parseFile($files[$fileType]);
    }

    /**
     * @return array
     */
    private function getFiles() : array
    {
        $files = [
            'global' => getenv('HOME').'/.git-pull-request.yml',
        ];

        try {
            if ($this->git->isInsideWorkTree()) {
                $workTree        = $this->git->getProjectRootDir();
                $files['shared'] = $workTree.'/.git-pull-request.yml';
                $files['local']  = $workTree.'/.git/.git-pull-request.yml';
            }
        } catch (RuntimeException $exception) {
            // The script can run without this part of the code.
        }

        return array_filter(
            $files,
            function ($item) {
                return (new Filesystem())->exists($item);
            }
        );
    }

    /**
     * @param string $file
     *
     * @return array
     */
    private function parseFile(string $file) : array
    {
        try {
            return Yaml::parse(file_get_contents($file));
        } catch (ParseException $exception) {
            return [];
        }
    }
}
